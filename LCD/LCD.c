...
static void fallingEdge(void) {
    HAL_GPIO_WritePin(GPIO_PORTC, E_Pin, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(GPIO_PORTC, E_Pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(GPIO_PORTC, E_Pin, GPIO_PIN_RESET);
    HAL_Delay(1);
}
	...
	static void send4Bits(char data) {
		HAL_GPIO_WritePin(GPIO_PORTB, DATA5_Pin, SET_IF(data&0x01));
		HAL_GPIO_WritePin(GPIO_PORTB, DATA6_Pin, SET_IF(data&0x02));
		HAL_GPIO_WritePin(GPIO_PORTB, DATA7_Pin, SET_IF(data&0x04));
		HAL_GPIO_WritePin(GPIO_PORTA, DATA8_Pin, SET_IF(data&0x08));
...
static void sendCommand(char cmd) {
		...
		send8Bits(cmd);
	#else
	    HAL_GPIO_WritePin(GPIO_PORTA, RS_Pin, GPIO_PIN_RESET);
		send4Bits(cmd >> 4);
		send4Bits(cmd);
	#endif
}
...
static void sendData(char data) {
		...
		send8Bits(data);
	#else
	    HAL_GPIO_WritePin(GPIO_PORTA, RS_Pin, GPIO_PIN_SET);
		send4Bits(data >> 4);
		send4Bits(data);
	#endif
}
